<main id="chat-container" 
      class="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 scroll-smooth pb-20 md:pb-8 transition-all duration-500 
      {{ Auth::user()->chat_view_mode === 'compact' ? 'compact-mode space-y-2 p-2 md:p-4' : 'space-y-6' }}">
    ...
    <div id="empty-state" class="flex flex-col items-center justify-center h-full opacity-60 mt-4">
        <div class="w-24 h-24 bg-slate-100 rounded-full flex items-center justify-center mb-4 text-slate-300">
            <i class="ri-cup-line text-4xl"></i>
        </div>
    ...
    <div id="chat-messages">
        @foreach($messages as $message)
            ...
            <div class="message-wrapper flex w-full {{ $isMe ? 'justify-end' : 'justify-start' }} animate-fade-up group relative" id="msg-{{ $message->id }}">
                <div class="message-body max-w-[85%] md:max-w-[65%] flex flex-col {{ $isMe ? 'items-end' : 'items-start' }}">
                    ...
                    <div class="relative group/bubble">
                        ...
                        <div class="bubble-content {{ $isMe ? 'bg-indigo-600 text-white rounded-tr-none shadow-indigo-100' : 'bg-white border border-slate-200 text-slate-700 rounded-tl-none' }} rounded-2xl shadow-sm px-4 py-3 text-[15px] md:text-base leading-relaxed {{ $message->is_sensitive ? 'blur-content' : '' }}">
                        ...
                        <div class="absolute {{ $isMe ? '-left-8' : '-right-8' }} top-2 opacity-0 group-hover/bubble:opacity-100 transition-opacity" x-data="{ open: false }">
                            ...
                            <div x-show="open" @click.outside="open = false" style="display: none;" 
                                ...
                                <button onclick="startReply({{ $message->id }}, '{{ $isMe ? 'ti mesmo' : ($message->is_anonymous ? 'Anónimo' : e($message->user->name)) }}')" 
                                ...
                                @if(auth()->id() === $message->user_id || auth()->user()->isModerator())
                                    @if(auth()->id() === $message->user_id && $message->created_at->diffInMinutes(now()) <= 5)
                                        <button onclick="startEdit({{ $message->id }})" class="w-full text-left px-3 py-2 text-xs font-bold text-indigo-600 hover:bg-indigo-50 flex items-center gap-2">
                    ...
                    <div class="message-meta-container flex items-center gap-2 mt-1 px-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                        @if(!$isMe)
    ...
    <button id="jump-to-bottom-btn" onclick="scrollToBottom(true)" class="fixed bottom-24 left-1/2 -translate-x-1/2 md:bottom-28 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-full shadow-xl z-50 flex items-center gap-2 text-xs font-bold transition-all transform translate-y-20 opacity-0 pointer-events-none border border-white/20">
        <i class="ri-arrow-down-line"></i>
        <span>Novas mensagens</span>
        <span id="unread-count-badge" class="bg-white text-indigo-600 px-1.5 rounded-full text-[10px] hidden">0</span>
    </button>
</main>
...
<div class="p-4 bg-white border-t border-slate-100">
    ...
    <form id="chat-form" class="relative flex items-end gap-2">
        ...
        <textarea id="messageInput" rows="1" class="w-full bg-slate-50 border-0 rounded-2xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all resize-none max-h-32" placeholder="Escreve a tua mensagem... (Shift+Enter para nova linha)"></textarea>
...
<style>
    ...
    .compact-mode .message-wrapper { margin-bottom: 0.25rem !important; }
    .compact-mode .bubble-content { padding: 0.5rem 0.75rem !important; font-size: 0.875rem !important; border-radius: 0.75rem !important; }
    .compact-mode .message-meta-container { margin-top: 0 !important; transform: scale(0.9); transform-origin: left top; }
    .compact-mode .sensitive-overlay { border-radius: 0.75rem !important; }
</style>