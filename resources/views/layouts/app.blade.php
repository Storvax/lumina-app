...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased relative">
        <div class="min-h-screen bg-gray-100 pb-20 md:pb-0"> @include('layouts.navigation')
        ...
        {{-- BOTÃO GLOBAL SOS (Floating Action Button) --}}
        <a href="{{ route('calm.crisis') }}" class="fixed bottom-6 right-6 z-50 flex items-center justify-center w-14 h-14 bg-rose-500 text-white rounded-full shadow-xl shadow-rose-500/30 hover:bg-rose-600 hover:scale-105 active:scale-95 transition-all group focus:outline-none focus:ring-4 focus:ring-rose-500/50" aria-label="Aceder ao Modo Crise de Emergência">
            <i class="ri-lifebuoy-line text-2xl group-hover:animate-pulse"></i>
        </a>

        {{-- TOAST GAMIFICATION (Subido para bottom-24 para não tapar o botão SOS) --}}
        @if(session('gamification.flames') || session('gamification.badge') || session('gamification.mission'))
            <div id="gamification-toast" class="fixed bottom-24 right-6 z-50 animate-fade-up">
                <div class="bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl border border-slate-700 flex items-center gap-4">
                    ...
                    @elseif(session('gamification.mission'))
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400">Ganhaste as tuas chamas!</p> </div>
